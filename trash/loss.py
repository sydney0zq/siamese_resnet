...
class YOLOloss():
    ...
    def forward(self):
        ...
        label = self.label.data.cpu().numpy()
        pred = self.pred.data.cpu().numpy()
        BSZ, ROW, COL = self.lsz[0], self.lsz[2], self.lsz[3]

        # Delta is the differential coefficient
        delta = self.pred[:, :self.num_box, :, :]
        self.diff = self.noobject_scale * delta     #NOTE
        avg_anyobj += np.sum(pred[:, :self.num_box, :, :])
                ...
                for col in range(COL):
                    ...
                    ###
                        ...
                        for i in range(self.num_box):
                            ...
                            delta = pred[i_pair, self.num_box+self.num_class*best_index:self.num_box+self.num_class*(best_index+1), row, col] - label[i_pair, 1:1+self.num_class, row, col]
                            self.diff[i_pair, self.num_box+self.num_class*best_index:self.num_box+self.num_class*(best_index+1), row, col] = self.class_scale * delta
                            self.loss += self.class_scale * np.sum(delta ** 2)

                            for key, value in enumerate(label[i_pair, 1:1+self.num_class, row, col]):
                                if val == 1:
                                    avg_cat += pred[i_pair, self.num_box+self.num_class*best_index+key, row, col]
                            avg_allcat += np.sum(pred[i_pair, self.num_box+self.num_class*best_index:self.num_box+self.num_class*(best_index+1), row, col])

                            # Compute iou between matched box and groundtruth
                            out = copy(pred[i_pair, self.num_box*(self.num_class+1)+4*best_index:self.num_box*(self.num_class+1)+4*(best_index+1), row, col])
                            out[0] /= COL
                            out[1] /= ROW
                            out[2] = out[2] ** 2
                            out[3] = out[3] ** 2
                            ...
                            delta = pred[i_pair, best_index, row, col] - 1.0
                            self.loss -= self.noobject_scale * (pred[i_pair, best_index, row, col] ** 2)
                            self.loss += self.object_scale * (delta ** 2)
                            avg_obj += pred[i_pair, best_index, row, col]

                            self.diff[i_pair, best_index, row, col] = self.object_scale * (pred[i_pair, best_index, row, col] - iou)

                            # Forward and backward of corrdinates
                            delta = pred[i_pair, self.num_box*(self.num_class+1)+4*best_index:self.num_box*(self.num_class+1)+4*best_index+2, row, col]
                            self.diff[i_pair, self.num_box*(self.num_class+1)+4*best_index:self.num_box*(self.num_class+1)+4*best_index+2, row, col] = self.coord_scale * delta
                            delta = pred[i_pair, self.num_box*(self.num_class+1)+4*best_index+2:self.num_box*(self.num_class+1)+4(best_index+1), row, col] - \
                                    label[i_pair, 3+self.num_class:, row, col] ** 0.5
                            self.diff[i_pair, self.num_box*(slef.num_class+1)+4*best_index+2:self.num_box*(self.num_class+1)+4*(best_index+1), row, col] = self.coord_scale * delta
                            
                            self.loss += (iou - 1.0) ** 2
                            avg_iou += iou
                            count_obj += 1
            if count_obj == 0:
                self.diff[i_pair, :self.num_box, :, :] = 0
            else:
                count += count_obj
            self.loss /= self.pred.size()[0]
        
            print ("Detection Avg IOU: {:.3f}, count: {:.3f}".format(avg_iou/(count+1), count))

            
        def np2variable(x, dtype=torch.FloatTensor):
            return Variable(torch.from_numpy(x).type(dtype), requires_grad=True, volatile=False)

        self.label, self.pred = np2variable(label), np2variable(pred)
        self.loss = np2variable(self.loss)
